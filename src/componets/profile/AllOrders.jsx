...
function AllOrders() {
  ...
  const handleStatusChange = async () => {
    if (selectedOrder && selectedStatus) {
      try {
        await dispatch(orderRefund({ id: selectedOrder?._id, status: selectedStatus })).unwrap();
        message.success("Order Refund Request successfully");
        dispatch(getUserOder(userId));
        setIsModalVisible(false);
      } catch (error) {
    ...
    <div className="w-full px-5">
          ...
          <div>
            <div className="flex items-center justify-between ">
              ...
              {selectedOrder.status === "Received" && (
                <button
                  onClick={() => {
                    setSelectedStatus("Requested Refund");
                    handleStatusChange();
                  }}
      ...
      >
        ...
        <form onSubmit={formik.handleSubmit}>
          ...
          <button type="submit" className="mt-2 p-2 bg-black text-white w-full">
            Submit Review
          </button>
        </form>
      </Modal>
